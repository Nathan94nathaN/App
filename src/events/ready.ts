...
export const name: Event["name"] = "ready",
  execute: Event["execute"] = async (client: Game) => {
    ...
    await client._rest.put(
      ...
      { body: client.commands.map((v: SlashCommand) => v.data) }
    );

    const currentDate = new Date(),
      cb = () => {
        (
          client.channels.cache.get(process.env.HOUR_CHANNEL_ID) as VoiceChannel
        ).setName(
          `⏰ (UTC +1) ${currentDate.toLocaleTimeString("fr-FR").slice(0, 5)}`
        );
      };
